#!/usr/bin/env bun
import { existsSync, statSync, readdirSync, unlinkSync, renameSync, mkdirSync, copyFileSync } from "fs";
...
async function processFile(
  ...
  if (!opts.keep) {
    const archiveDir = resolve(process.cwd(), "Archive", "Original-Images");
    if (!existsSync(archiveDir)) mkdirSync(archiveDir, { recursive: true });
    
    const ext = extname(absInput);
    const baseFile = basename(absInput, ext);
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const archivePath = join(archiveDir, `${baseFile}_${timestamp}${ext}`);
    
    if (absInput !== output) {
      renameSync(absInput, archivePath);
    } else {
      copyFileSync(absInput, archivePath);
    }
  }
...
Options:
  ...
  -f, --format <fmt>    Output format: webp, png, jpeg (default: jpeg)
  -q, --quality <n>     Quality 0-100 (default: 65)

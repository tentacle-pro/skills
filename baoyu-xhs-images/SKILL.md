...
Each session creates an independent directory named by content slug. According to the vault's PARA structure (see [[/AGENTS.md]]), these should be placed in `Projects/` if they are active projects, or `Archives/Projects/` if completed.

Default base path: `Projects/xhs-posts/{topic-slug}/`

```
Projects/xhs-posts/{topic-slug}/
...
**Conflict Resolution**:
If `Projects/xhs-posts/{topic-slug}/` already exists:
...
- [ ] Step 1: Analyze content → analysis.md (In [[Projects/xhs-posts/{slug}/]])
...
```bash
# Vault-specific location (Recommended)
test -f .agents/skills/baoyu-xhs-images/EXTEND.md && echo "vault"
# Global locations
...
│                        Path                        │     Location      │
├────────────────────────────────────────────────────┼───────────────────┤
│ .agents/skills/baoyu-xhs-images/EXTEND.md           │ Vault Skill dir   │
├────────────────────────────────────────────────────┼───────────────────┤
...
Use AskUserQuestion with ALL questions in ONE call. See `.agents/skills/baoyu-xhs-images/references/config/first-time-setup.md` for question details.

**EXTEND.md Supports**: Watermark | Preferred style/layout | Custom style definitions | Language preference

Schema: `.agents/skills/baoyu-xhs-images/references/config/preferences-schema.md`
...
1. **Save source content** (if not already a file):
   ...
   - If user pastes content: save to `source.md` in target directory `Projects/xhs-posts/{slug}/`
   - **Backup rule**: If `source.md` exists, rename to `source-backup-YYYYMMDD-HHMMSS.md`
2. Read source content
3. **Deep analysis** following `.agents/skills/baoyu-xhs-images/references/workflows/analysis-framework.md`:
...
7. **Save to `analysis.md`** in `Projects/xhs-posts/{slug}/`
...
Generate single outline using recommended strategy + style → save to `outline.md` in `Projects/xhs-posts/{slug}/` → Step 3.
...
- Use `[[Projects/xhs-posts/{slug}/analysis.md]]` for internal analysis references.

Reference: `.agents/skills/baoyu-xhs-images/references/workflows/outline-template.md`
...
**After response**: Save selected/merged outline to `outline.md` in `Projects/xhs-posts/{slug}/` with confirmed style and elements → Step 3.
...
2. **Use image 1 as `--ref` for ALL remaining images** (2, 3, ..., N)
   ...
   - For image generation, use the `baoyu-image-gen` skill if available.
...
1. Save prompt to `Projects/xhs-posts/{slug}/prompts/NN-{type}-[slug].md` (in user's preferred language)
...
```
Reference: `.agents/skills/baoyu-xhs-images/references/config/watermark-guide.md`

**Image Generation Skill Selection**:
- Use `baoyu-image-gen` (Recommended) or other available image generation skills.
...
Location: [[Projects/xhs-posts/{slug}/]]
Images: N total
...
Detailed templates in `.agents/skills/baoyu-xhs-images/references/` directory:

**Elements** (Visual building blocks):
- `.agents/skills/baoyu-xhs-images/references/elements/canvas.md` - Aspect ratios, safe zones, grid layouts
- `.agents/skills/baoyu-xhs-images/references/elements/image-effects.md` - Cutout, stroke, filters
- `.agents/skills/baoyu-xhs-images/references/elements/typography.md` - Decorated text (花字), tags, text direction
- `.agents/skills/baoyu-xhs-images/references/elements/decorations.md` - Emphasis marks, backgrounds, doodles, frames

**Presets** (Style presets):
- `.agents/skills/baoyu-xhs-images/references/presets/<style>.md` - Element combination definitions (cute, notion, warm...)
- `.agents/skills/baoyu-xhs-images/references/style-presets.md` - Preset shortcuts (style + layout combos)

**Workflows** (Process guides):
- `.agents/skills/baoyu-xhs-images/references/workflows/analysis-framework.md` - Content analysis framework
- `.agents/skills/baoyu-xhs-images/references/workflows/outline-template.md` - Outline template with layout guide
- `.agents/skills/baoyu-xhs-images/references/workflows/prompt-assembly.md` - Prompt assembly guide

**Config** (Settings):
- `.agents/skills/baoyu-xhs-images/references/config/preferences-schema.md` - EXTEND.md schema
- `.agents/skills/baoyu-xhs-images/references/config/first-time-setup.md` - First-time setup flow
- `.agents/skills/baoyu-xhs-images/references/config/watermark-guide.md` - Watermark configuration

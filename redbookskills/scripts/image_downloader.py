...
DEFAULT_TEMP_ROOT = os.path.join(SKILL_ROOT, "images", "publish_temp")
...
class ImageDownloader:
    ...
    def __init__(self, temp_dir: str | None = None):
        if temp_dir:
            ...
            os.makedirs(DEFAULT_TEMP_ROOT, exist_ok=True)
            self.temp_dir = os.path.join(DEFAULT_TEMP_ROOT, f"{TEMP_DIR_PREFIX}{uuid.uuid4().hex[:8]}")
            os.makedirs(self.temp_dir, exist_ok=True)
            self._owns_dir = True
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: uv run .agents/skills/redbookskills/scripts/image_downloader.py <url1> [url2] ...")
        sys.exit(1)

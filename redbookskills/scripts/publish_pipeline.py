...
Usage:
    ...
    uv run .agents/skills/redbookskills/scripts/publish_pipeline.py --title "标题" --content "正文" --image-urls URL1 URL2
    uv run .agents/skills/redbookskills/scripts/publish_pipeline.py --title-file t.txt --content-file body.txt --image-urls URL1

    # Fill form only for manual review (preview mode)
    uv run .agents/skills/redbookskills/scripts/publish_pipeline.py --title "标题" --content "正文" --image-urls URL1 --preview

    # Headless mode (no GUI window) - faster for automated publishing
    uv run .agents/skills/redbookskills/scripts/publish_pipeline.py --headless --title-file t.txt --content-file body.txt --image-urls URL1

    # Publish to a specific account
    uv run .agents/skills/redbookskills/scripts/publish_pipeline.py --account myaccount --title "标题" --content "正文" --image-urls URL1

    # Explicit auto-publish flag (optional compatibility flag)
    uv run .agents/skills/redbookskills/scripts/publish_pipeline.py --title "标题" --content "正文" --image-urls URL1 --auto-publish

    # Prefer reusing existing tab (reduce focus switching in headed mode)
    uv run .agents/skills/redbookskills/scripts/publish_pipeline.py --reuse-existing-tab --title "标题" --content "正文" --image-urls URL1

    # Use local image files instead of URLs
    uv run .agents/skills/redbookskills/scripts/publish_pipeline.py --title "标题" --content "正文" --images img1.jpg img2.jpg
    # Skip local file check (for WSL/remote CDP + Windows/UNC paths)
    uv run .agents/skills/redbookskills/scripts/publish_pipeline.py --title "标题" --content "正文" --images "\\\\wsl.localhost\\Ubuntu\\home\\me\\a.jpg" --skip-file-check

    # Publish a video (local file)
    uv run .agents/skills/redbookskills/scripts/publish_pipeline.py --title "标题" --content "正文" --video video.mp4

    # Publish a video (from URL)
    uv run .agents/skills/redbookskills/scripts/publish_pipeline.py --title "标题" --content "正文" --video-url "https://example.com/video.mp4"

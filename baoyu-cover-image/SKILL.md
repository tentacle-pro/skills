...
Output directory MUST be: `Assets/Cover-Images/{topic-slug}/`
(As this vault is for a text & image content creator, all generated cover images and their assets live here uniformly to decouple them from the article.)

```
Assets/Cover-Images/{topic-slug}/
...
```
Input → Analyze + Save Refs → [Output Dir] → [Confirm: 6 Dimensions] → Prompt → Generate → Complete
                                             ↓
...
5. **Detect language**: Compare source and user input
...
### Step 4: Generate Image (using baoyu-image-gen)

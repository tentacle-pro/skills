...
```
- [ ] Step 1: Pre-check (source, output dir, references)
...
1. Confirm source article path or pasted content.
2. Create output directory under `Assets/Article-Illustrations/{topic-slug}/`.
3. Ensure `prompts/` subdirectory exists before generation.
4. Load style/type references from `references/`.
...
| Q4: Language | When source language ≠ user language |
...
7. For each prompt file, generate with unified command:
	...
	```bash
	bun .agents/skills/baoyu-image-gen/scripts/main.ts --promptfiles <output-dir>/prompts/NN-{type}-{slug}.md --image <output-dir>/NN-{type}-{slug}.png --ar 16:9
	```
...
```
Assets/Article-Illustrations/{topic-slug}/
├── source-{slug}.{ext}

...
|-------|------------------|------|
| File path | `Assets/Article-Illustrations/{topic-slug}/` | → 1.2 |
| Pasted content | `Assets/Article-Illustrations/{topic-slug}/` | → 1.4 |
...
Check existing state, then ask ALL needed questions in ONE AskUserQuestion call (max 4 questions).
...
Use skill defaults when user has not specified preferences.
...
### Q4: Image Text Language ⚠️ REQUIRED when article language ≠ user language

Detect article language from content. If different from user language preference, ask:
- Article language (match article content) (Recommended)
- User language preference

**Skip only if**: Article language matches user language preference.
